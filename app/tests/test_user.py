...
def test_update_user_in_db_name(test_client):
    ...
    # Consulta la base de datos el usuario fue actualizado
    updated_user = Users.query.filter_by(id=1).first()
    assert updated_user is not None
    assert updated_user.name == "Carla Martin Updated"


def test_remove_user_from_db(test_client):
    ...
    delete_user = Users.query.filter_by(id=1).first()
